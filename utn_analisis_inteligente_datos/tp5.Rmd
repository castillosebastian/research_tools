...
datos %>% 
  ...
  group_by(grupo, variables) %>% 
  summarise(medias = round(mean(cantidad, na.rm=T), digits = 2)) %>%
  #mutate(medias_scaled = scales::rescale(medias)) %>% 
  ggplot(aes(x = variables, y = medias)) +
...
# Analisis Discriminante Cuadrático Robusto (RQDA)\>falta normalidad ..PENDIENTE
...
```{r, eval=LDA}
datos %>% 
  pivot_longer(names_to = 'variables', values_to = 'cantidad', -variable_factor_svm) %>% 
  rename(grupo = variable_factor_svm) %>% 
  ggplot(aes(x=variables, y=cantidad, fill=grupo)) + 
  geom_boxplot() +
  facet_wrap(~variables, scales = 'free')
```
...
```{r, eval=LDA}
variable_factor_lda = "Clase"
datos = readxl::read_xlsx("barotro.xlsx") 
#factorizo variable factor
datos[[{{variable_factor_lda}}]] = as.factor(datos[[{{variable_factor_lda}}]]) 
# resumente datos
str(datos)
...
##### ARMAR CON VARIOS KERNELS!!!!

##### ingenuo no CROSVALIDATION!!!

##### lda O qDA ROBUSTO! VER CUANDO: ELEGIR MATRIZ CON VARIANZA EN FORMA ROBUSTA.
